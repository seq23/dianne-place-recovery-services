...
const blogUrls = calendar.items.map((item) => ({
...
fs.writeFileSync(
  ...
  `${JSON.stringify({ generatedAt: new Date().toISOString(), urlCount: urls.length, blogCount: blogUrls.length }, null, 2)}\n`
);
console.log(`[seo:sitemap] wrote ${urls.length} URLs`);
