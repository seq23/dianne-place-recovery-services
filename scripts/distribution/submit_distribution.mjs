...
const manifest = readJson('data/distribution/distribution_manifest.json');
const batchUrls = readLines('data/distribution/indexnow_batch.txt');
const priorityUrls = readLines('data/distribution/priority_urls.txt');
if (!batchUrls.length) throw new Error('Distribution batch is empty.');
...
async function submitIndexNow() {
  ...
  if (!key || !keyLocation) return { status: 'NOT_CONFIGURED', attempted: false, submitted_urls: 0 };
  try {
    const payload = { host, key, keyLocation, urlList: batchUrls.slice(0, 10000) };
    const response = await fetch('https://api.indexnow.org/indexnow', {
      method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload)
    });
    return { status: response.ok ? 'SUCCESS' : 'FAILED', attempted: true, http_status: response.status, submitted_urls: payload.urlList.length };
  } catch (error) {
    return { status: 'FAILED', attempted: true, submitted_urls: 0, error: String(error?.message || error) };
  }
}
...
async function runGsc() {
  if (!process.env.GSC_SERVICE_ACCOUNT_JSON || !process.env.GSC_SITE_URL) {
    return {
      sitemap: { status: 'NOT_CONFIGURED', attempted: false },
      inspection: { status: 'NOT_CONFIGURED', attempted: false, requested_urls: priorityUrls.length, inspected_urls: 0, results: [] }
    };
  }
  try {
    const token = await getGoogleToken();
    const sitemapEndpoint = `https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(process.env.GSC_SITE_URL)}/sitemaps/${encodeURIComponent(manifest.sitemap_url)}`;
    const sitemapResponse = await fetch(sitemapEndpoint, { method: 'PUT', headers: { authorization: `Bearer ${token}` } });
    const sitemap = {
      status: sitemapResponse.ok ? 'SUCCESS' : 'FAILED', attempted: true,
      http_status: sitemapResponse.status, site_url: process.env.GSC_SITE_URL, sitemap_url: manifest.sitemap_url
    };
    ...
    for (const url of priorityUrls.slice(0, limit)) {
      try {
        const response = await fetch('https://searchconsole.googleapis.com/v1/urlInspection/index:inspect', {
          ...
          body: JSON.stringify({ inspectionUrl: url, siteUrl: process.env.GSC_SITE_URL, languageCode: 'en-US' })
        });
        const body = await response.json().catch(() => ({}));
        const indexResult = body?.inspectionResult?.indexStatusResult || {};
        results.push({
          url, status: response.ok ? 'SUCCESS' : 'FAILED', http_status: response.status,
          verdict: indexResult.verdict || 'UNKNOWN', coverage_state: indexResult.coverageState || 'UNKNOWN',
          indexing_state: indexResult.indexingState || 'UNKNOWN', last_crawl_time: indexResult.lastCrawlTime || null
        });
      } catch (error) {
        results.push({ url, status: 'FAILED', error: String(error?.message || error) });
      }
    }
    ...
    return {
      ...
      inspection: {
        ...
        attempted: true, requested_urls: priorityUrls.length, inspected_urls: results.length, failed_urls: failures, results
      }
    };
  } catch (error) {
    const message = String(error?.message || error);
    return {
      sitemap: { status: 'FAILED', attempted: true, error: message },
      inspection: { status: 'FAILED', attempted: true, requested_urls: priorityUrls.length, inspected_urls: 0, results: [], error: message }
    };
  }
}
...
const receipt = {
  ...
  publication_trigger: manifest.publication_trigger,
  chain: manifest.chain,
  sitemap_refresh: { status: 'SUCCESS', sitemap_url: manifest.sitemap_url, sitemap_sha256: manifest.sitemap_sha256, url_count: manifest.url_count },
  indexnow,
  gsc_sitemap_submission: gsc.sitemap,
  priority_url_inspection: gsc.inspection,
  ...
  provider_success_claimed: providerSuccess,
...
const feedback = {
  ...
  sitemap_refresh_status: receipt.sitemap_refresh.status,
  indexnow_status: indexnow.status,
  gsc_sitemap_status: gsc.sitemap.status,
  gsc_inspection_status: gsc.inspection.status,
